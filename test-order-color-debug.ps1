...
try {
    ...
    Write-Host "`n✅ Respuesta completa:" -ForegroundColor Green
    $response | ConvertTo-Json -Depth 10 | Write-Host -ForegroundColor Cyan
    
    $orderId = $response.order_id
    Write-Host "`n📦 Order ID: $orderId" -ForegroundColor Magenta
    
    if ($orderId) {
        Write-Host "`n📋 Consultando orden..." -ForegroundColor Cyan
        Start-Sleep -Seconds 2
        $order = Invoke-RestMethod -Uri "$endpoint/$orderId" -Method GET
        
        Write-Host "`n✅ Orden consultada:" -ForegroundColor Green
        $order | ConvertTo-Json -Depth 10 | Write-Host -ForegroundColor White
        
        Write-Host "`n🔍 Verificando campos en el primer ticket:" -ForegroundColor Yellow
