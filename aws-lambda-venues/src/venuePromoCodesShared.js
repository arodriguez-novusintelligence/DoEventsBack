...
const TABLE = process.env.VENUE_PROMO_CODES_TABLE || "VenuePromoCodes-dev";
...
async function queryVenuePromoItems(venueId) {
...
async function getVenueOwnerId(venueId) {
  const result = await dynamodb
    .get({
    ...
    .promise();
  return result.Item?.ownerUserId || result.Item?.owner_user_id || null;
}
...
async function assertVenueOwnerAccess(venueId, userId) {
  if (!userId) {
...
async function assertOwnerOrPlatformAdmin(venueId, userId) {
  if (!userId) {
    const error = new Error("Unauthorized");
    error.statusCode = 401;
    throw error;
...
function buildPromoPayload(items) {
  ...
  codeRecords.forEach((record, code) => {
    ...
    redemptions[code] = {
      ...
      rentalDays: Number(record.rentalDays || 1),
...
async function putBatchWithCodes(venueId, batch, organizerId) {
