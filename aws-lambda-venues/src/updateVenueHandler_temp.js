 ...
 * Crea un ticket instance por cada boleta disponible en cada categorÃ­a
...
async function generateTicketsDistribution(
  ...
  console.log(
    `ðŸŽ« Generando TicketsDistribution para ${categories.length} categorÃ­as...`
  );
  ...
  for (const category of categories) {
    ...
    if (cantidadTickets <= 0) {
      console.log(
        `âš ï¸ CategorÃ­a ${category.categoria} no tiene tickets, omitiendo...`
      );
      continue;
    }
    ...
    console.log(
      `âœ… ${cantidadTickets} tickets creados para categorÃ­a ${category.categoria}`
    );
  }

  // Guardar en TicketsDistribution por lotes (mÃ¡ximo 25 por batch)
  if (distributionItems.length > 0) {
    ...
    console.log(
      `âœ… ${distributionItems.length} distribuciones creadas en TicketsDistribution`
    );
  }
...
/**
 * Sincroniza TicketsDistribution con las categorÃ­as actuales del venue
 * Actualiza las distribuciones existentes y crea nuevas si es necesario
 * NOTA: TicketsDistribution usa clave compuesta (id, createDate)
 */
async function syncTicketsDistribution(
  ...
  console.log(
    `ðŸ”„ Sincronizando TicketsDistribution para ${categories.length} categorÃ­as...`
  );

  // 1. Obtener distribuciones existentes por eventId
  const existingDistributions = await dynamodb
    .query({
  ...
  const existingMap = {};
  for (const dist of existingDistributions.Items || []) {
    existingMap[dist.boletaId] = dist;
  }

  console.log(
    `ðŸ“Š Encontradas ${existingDistributions.Items.length} distribuciones existentes`
  );

  const distributionsToCreate = [];
  const distributionsToUpdate = [];

  for (const category of categories) {
    const distributionId = category.distributionId || uuidv4();
    const createDate = category.distributionCreateDate || now;
    const cantidadTickets = category.cantidadTickets || 0;
    const existingDist = existingMap[category.id];
    ...
    if (!existingDist) {
      // Nueva categorÃ­a - crear distribuciÃ³n
      console.log(`âž• Creando nueva distribuciÃ³n para ${category.categoria}`);
      const ticketsArray = [];
      for (let i = 0; i < cantidadTickets; i++) {
        ticketsArray.push({
          ticketInstanceId: uuidv4(),
      ...
      // CategorÃ­a existente - verificar si cambiÃ³ cantidad
      const existingTickets = existingDist.tickets || [];
      const currentAvailable = existingTickets.filter(
        (t) => t.ticketStatus === "AVAILABLE"
      ).length;
      const reserved = existingTickets.filter(
        (t) => t.ticketStatus === "RESERVED"
      ).length;
      const sold = existingTickets.filter(
        (t) => t.ticketStatus === "SOLD" || t.ticketStatus === "USED"
      ).length;

      console.log(
        `ðŸ“Š CategorÃ­a ${category.categoria}: Total=${existingTickets.length}, Disponibles=${currentAvailable}, Reservados=${reserved}, Vendidos=${sold}`
      );

      if (cantidadTickets !== existingTickets.length) {
        console.log(
          `ðŸ”„ Ajustando cantidad de ${existingTickets.length} a ${cantidadTickets}`
        );
        ...
        if (cantidadTickets > existingTickets.length) {
          ...
          const toAdd = cantidadTickets - existingTickets.length;
          console.log(`âž• Agregando ${toAdd} tickets nuevos`);
          for (let i = 0; i < toAdd; i++) {
            updatedTickets.push({
              ...
              createDate: existingDist.createDate,
            });
          }
        } else {
          // Reducir tickets (solo si hay suficientes disponibles)
          const toRemove = existingTickets.length - cantidadTickets;
          if (toRemove > currentAvailable) {
            console.log(
              `âš ï¸ No se puede reducir: se requieren ${toRemove} disponibles pero solo hay ${currentAvailable}`
            );
            continue;
          }

          console.log(`âž– Eliminando ${toRemove} tickets disponibles`);
          let removed = 0;
          updatedTickets = updatedTickets.filter((t) => {
            if (t.ticketStatus === "AVAILABLE" && removed < toRemove) {
              removed++;
              return false;
            }
            return true;
        ...
        // Actualizar distribuciÃ³n usando clave compuesta
        distributionsToUpdate.push({
          id: existingDist.id, // PK
          createDate: existingDist.createDate, // SK
  ...
  if (distributionsToCreate.length > 0) {
    const chunkSize = 25;
    for (let i = 0; i < distributionsToCreate.length; i += chunkSize) {
      const chunk = distributionsToCreate.slice(i, i + chunkSize);
      await dynamodb
        .batchWrite({
          RequestItems: {
            TicketsDistribution: chunk,
    ...
    console.log(
      `âœ… ${distributionsToCreate.length} nuevas distribuciones creadas`
    );
  }

  // Ejecutar actualizaciones con clave compuesta
  for (const update of distributionsToUpdate) {
    await dynamodb
      .update({
        TableName: "TicketsDistribution",
        Key: {
          id: update.id, // PK
          createDate: update.createDate, // SK
        },
  ...
  if (distributionsToUpdate.length > 0) {
    console.log(
      `âœ… ${distributionsToUpdate.length} distribuciones actualizadas`
    );
  }

  return {
    created: distributionsToCreate.length,
    updated: distributionsToUpdate.length,
  };
}

/**
 * Handler mejorado para actualizar venues con comparaciÃ³n inteligente
 * Soporta:
 * - ComparaciÃ³n de datos existentes vs nuevos
 * - EliminaciÃ³n en batch de categorÃ­as, pisos y sillas
 * - ActualizaciÃ³n masiva de elementos
 * - SincronizaciÃ³n automÃ¡tica con tabla Tickets
 * - PreservaciÃ³n de datos de ventas (soldTickets, reservedTickets)
 * - SincronizaciÃ³n bidireccional Venue_Category â†” Tickets
 */
exports.handler = async (event) => {
  try {
    ...
    // PARTE 1: ACTUALIZAR CAMPOS BÃSICOS DEL VENUE
    console.log("ðŸ“ Actualizando campos bÃ¡sicos del venue...");
    await updateBasicVenueFields(venueId, body, existing.Item, userId, now);

    // PARTE 2: PROCESAR FLOORS (pisos) con auto-eliminaciÃ³n
    ...
    // PARTE 3: PROCESAR CATEGORÃAS INDEPENDIENTES
    ...
    // PARTE 5: SINCRONIZACIÃ“N AUTOMÃTICA CON TABLA TICKETS
    // Recolectar todas las categorÃ­as finales del venue y sincronizar con Tickets
    let finalTicketSync = null;
    if (eventId) {
      console.log(
        "ðŸ”„ Iniciando sincronizaciÃ³n automÃ¡tica con tabla Tickets..."
      );
...
/**
 * Actualiza solo los campos bÃ¡sicos del venue que han cambiado
 */
async function updateBasicVenueFields(
  ...
  allowedFields.forEach((field) => {
    if (
      ...
      console.log(`âœï¸ Campo modificado: ${field}`);
    }
  });
  ...
  if (updateExpressions.length > 0) {
    ...
    console.log(
      `âœ… Actualizando ${updateExpressions.length - 1} campos del venue`
    );
    ...
    console.log("âš ï¸ No hay campos bÃ¡sicos para actualizar");
  }
}

/**
 * Procesa floors con sincronizaciÃ³n automÃ¡tica: elimina floors que ya no estÃ¡n en el array
 */
async function processFloorsWithSync(
  ...
  // Si no se envÃ­a el array floors, no hacer cambios (mantener existentes)
  if (newFloors === undefined) {
    console.log("âš ï¸ Array floors no enviado, manteniendo floors existentes");
    return {
      floorsProcessed: 0,
  ...
  // Si se envÃ­a array vacÃ­o, eliminar todos los floors
  if (!Array.isArray(newFloors)) {
    console.log("âš ï¸ floors debe ser un array");
    return {
      floorsProcessed: 0,
      categoriesProcessed: 0,
      seatsProcessed: 0,
  ...
  console.log(`ðŸ¢ Procesando ${newFloors.length} floors con sincronizaciÃ³n...`);
  ...
  // 2. Detectar floors a eliminar (existen pero no estÃ¡n en el nuevo array)
  ...
  if (floorsToDelete.length > 0) {
    console.log(
      `ðŸ—‘ï¸ Auto-eliminando ${floorsToDelete.length} floors que ya no estÃ¡n en el array:`,
      floorsToDelete
    );
  ...
  if (newFloors.length === 0) {
    console.log("âœ… Array floors vacÃ­o - todos los floors eliminados");
    return {
      floorsProcessed: 0,
      categoriesProcessed: 0,
      seatsProcessed: 0,
  ...
  console.log(`ðŸ¢ Procesando ${newFloors.length} floors...`);
  ...
  for (const floorData of newFloors) {
    ...
    if (isNewFloor) {
      ...
      console.log(`âž• Creando nuevo floor: ${floorData.name}`);
    } else {
      console.log(`âœï¸ Actualizando floor: ${floorData.name}`);
    }
    ...
    // Procesar categorÃ­as del floor
...
/**
 * Procesa elementos de un floor (elementos decorativos, etc) con sincronizaciÃ³n
 */
async function processFloorElements(floorId, venueId, elements, userId, now) {
  // Si elements es undefined, no hacer cambios
  if (elements === undefined) {
    console.log(
      `âš ï¸ Array elements no enviado para floor ${floorId}, manteniendo existentes`
    );
    return;
  }

  if (!Array.isArray(elements)) {
    console.log(`âš ï¸ elements debe ser un array para floor ${floorId}`);
    return;
  }

  // 1. Obtener elementos existentes del floor
  const existingElementsResult = await dynamodb
    .query({
  ...
  if (elementsToDelete.length > 0) {
    console.log(
      `ðŸ—‘ï¸ Auto-eliminando ${elementsToDelete.length} elementos del floor ${floorId}`
    );
    const batchSize = 25;
  ...
  // 3. Si array vacÃ­o, solo eliminamos
  if (elements.length === 0) {
    console.log(
      `âœ… Array elements vacÃ­o - todos los elementos eliminados del floor ${floorId}`
    );
    return;
  }

  console.log(
    `ðŸŽ¨ Procesando ${elements.length} elementos para floor ${floorId}`
  );
  ...
  for (let i = 0; i < elements.length; i += batchSize) {
    ...
    await dynamodb
      .batchWrite({
        RequestItems: {
          Venue_Element: putRequests,
        },
      })
      .promise();
  }

  console.log(`âœ… ${elements.length} elementos procesados`);
}

/**
 * Procesa categorÃ­as de un floor especÃ­fico con sincronizaciÃ³n
 */
async function processFloorCategories(
  ...
  if (categories === undefined) {
    console.log(
      `âš ï¸ Array categories no enviado para floor ${floorId}, manteniendo existentes`
    );
    return { categoriesProcessed: 0, seatsProcessed: 0 };
  }

  if (!Array.isArray(categories)) {
    console.log(`âš ï¸ categories debe ser un array para floor ${floorId}`);
    return { categoriesProcessed: 0, seatsProcessed: 0 };
  }

  // 1. Obtener categorÃ­as existentes del floor
  const existingCategoriesResult = await dynamodb
    .query({
  ...
  // 2. Detectar categorÃ­as a eliminar
  ...
  if (categoriesToDelete.length > 0) {
    console.log(
      `ðŸ—‘ï¸ Auto-eliminando ${categoriesToDelete.length} categorÃ­as del floor ${floorId}`
    );

    // Eliminar seats de cada categorÃ­a primero
    for (const categoryId of categoriesToDelete) {
      await deleteCategorySeats(categoryId);
    }

    // Eliminar categorÃ­as en batch
    const batchSize = 25;
  ...
  if (categories.length === 0) {
    console.log(
      `âœ… Array categories vacÃ­o - todas las categorÃ­as eliminadas del floor ${floorId}`
    );
    return { categoriesProcessed: 0, seatsProcessed: 0 };
  }

  console.log(
    `ðŸ“¦ Procesando ${categories.length} categorÃ­as para floor ${floorId}`
  );
  ...
  for (const categoryData of categories) {
    ...
    const isNew = !categoryData.categoryId;
    ...
    if (isNew) {
      ...
      console.log(`âž• Creando categorÃ­a: ${categoryData.name}`);
    } else {
      console.log(`âœï¸ Actualizando categorÃ­a: ${categoryData.name}`);
    }
    ...
    // Procesar asientos de la categorÃ­a con sincronizaciÃ³n
    if (hasSeating) {
...
/**
 * Procesa asientos en batch para una categorÃ­a con sincronizaciÃ³n
 */
async function processSeatsWithSync(
  ...
  if (seats === undefined) {
    console.log(
      `âš ï¸ Array seats no enviado para categorÃ­a ${categoryId}, manteniendo existentes`
    );
    return 0;
  }

  if (!Array.isArray(seats)) {
    console.log(`âš ï¸ seats debe ser un array para categorÃ­a ${categoryId}`);
    return 0;
  }

  // 1. Obtener seats existentes de la categorÃ­a
  ...
  const existingSeatIds = (existingSeatsResult.Items || []).map(
    (s) => s.seatId
  );
  ...
  if (seatsToDelete.length > 0) {
    console.log(
      `ðŸ—‘ï¸ Auto-eliminando ${seatsToDelete.length} asientos de la categorÃ­a ${categoryId}`
    );
    const batchSize = 25;
    for (let i = 0; i < seatsToDelete.length; i += batchSize) {
      const batch = seatsToDelete.slice(i, i + batchSize);
  ...
  if (seats.length === 0) {
    console.log(
      `âœ… Array seats vacÃ­o - todos los asientos eliminados de categorÃ­a ${categoryId}`
    );
    return 0;
  }

  console.log(
    `ðŸ’º Procesando ${seats.length} asientos para categorÃ­a ${categoryId}`
  );
  ...
  console.log(`âœ… ${processedCount} asientos procesados`);
  return processedCount;
}

/**
 * Procesa categorÃ­as independientes (no asociadas a floors)
 */
async function processIndependentCategories(
  ...
  console.log(
    `ðŸ“¦ Procesando ${categories.length} categorÃ­as independientes...`
  );
  ...
  for (const cat of categories) {
    ...
    // Si hasSeating y tiene floorId, crear en Venue_Category
    if (hasSeating && cat.floorId) {
      console.log(
        `ðŸ’º Vinculando categorÃ­a ${cat.name} con floor ${cat.floorId}`
      );
      ...
      // Procesar seats si existen
      if (cat.seats && Array.isArray(cat.seats) && cat.seats.length > 0) {
        const count = await processSeats(
          categoryId,
          cat.floorId,
          venueId,
          cat.seats,
 ...
 * Preserva datos importantes como reservedTickets y soldTickets
 */
async function updateTicketsTable(
  ...
  console.log(
    `ðŸŽ« Actualizando tabla Tickets con ${categories.length} categorÃ­as`
  );
  ...
  if (existingTickets.Items && existingTickets.Items.length > 0) {
    ...
    const existingBoletas = existingTickets.Items[0].boletas || [];
    console.log(`ðŸ“ Actualizando registro existente de tickets: ${ticketId}`);

    // Crear mapa de categorÃ­as existentes por ID para preservar datos
    ...
    // Combinar nuevas categorÃ­as con datos existentes
    const mergedCategories = categories.map((newCat) => {
      ...
      if (existingCat) {
        ...
        console.log(
          `ðŸ”„ Preservando datos de ventas para categorÃ­a: ${newCat.categoria}`
        );
        return {
          ...newCat,
          ...
          distributionId: existingCat.distributionId || newCat.distributionId,
          distributionCreateDate:
            existingCat.distributionCreateDate || newCat.distributionCreateDate,
        };
      }

      // Nueva categorÃ­a, usar valores por defecto
      return newCat;
    });
    ...
    await dynamodb
      .update({
        ...
        Key: { id: ticketId },
        UpdateExpression:
          "SET boletas = :boletas, updatedAt = :updatedAt, hasSeating = :hasSeating, venueId = :venueId",
        ExpressionAttributeValues: {
          ":boletas": mergedCategories,
          ":updatedAt": now,
    ...
    action = "updated";
    console.log(`âœ… Tickets actualizados correctamente con datos preservados`);
  } else {
    // Crear nuevo registro
    ticketId = uuidv4().substring(0, 10);
    console.log(`âž• Creando nuevo registro de tickets: ${ticketId}`);
    ...
    action = "created";
    console.log(`âœ… Tickets creados correctamente`);
  }
...
async function processBatchDeletions(venueId, deletions, eventId) {
  ...
  if (!deletions || Object.keys(deletions).length === 0) {
    console.log("âš ï¸ No hay eliminaciones para procesar");
    return results;
  }

  console.log("ðŸ—‘ï¸ Procesando eliminaciones en batch...");
  ...
  if (
    ...
    console.log(`ðŸ—‘ï¸ Eliminando ${deletions.floors.length} floors...`);
  ...
  if (
    ...
    console.log(`ðŸ—‘ï¸ Eliminando ${deletions.categories.length} categorÃ­as...`);
    ...
    console.log(`âœ… ${deletions.categories.length} categorÃ­as eliminadas`);
  }
  ...
  if (
    ...
    console.log(`ðŸ—‘ï¸ Eliminando ${deletions.seats.length} asientos...`);
    ...
    console.log(`âœ… ${deletions.seats.length} asientos eliminados`);
  }
  ...
  if (
    ...
    console.log(`ðŸ—‘ï¸ Eliminando ${deletions.elements.length} elementos...`);
    ...
    console.log(`âœ… ${deletions.elements.length} elementos eliminados`);
  }
...
/**
 * Elimina un floor y todos sus elementos relacionados (categorÃ­as, seats, elements)
 */
async function deleteFloor(floorId, venueId) {
  console.log(`ðŸ—‘ï¸ Eliminando floor ${floorId} y sus elementos relacionados...`);

  // Eliminar categorÃ­as del floor
  ...
  if (categories.Items && categories.Items.length > 0) {
    // Eliminar seats de cada categorÃ­a
  ...
  console.log(`âœ… Floor ${floorId} eliminado completamente`);
}

/**
 * Elimina todos los seats de una categorÃ­a
 */
...
/**
 * Sincroniza todas las categorÃ­as del venue con la tabla Tickets
 * Recolecta categorÃ­as de todos los floors y actualiza Tickets automÃ¡ticamente
 * Preserva datos de ventas y reservas existentes
 */
async function syncAllCategoriesToTickets(
  venueId,
  eventId,
  hasSeating,
  userId,
  ...
  console.log("ðŸ” Obteniendo todas las categorÃ­as del venue...");

  // 1. Obtener todas las categorÃ­as actuales de Venue_Category
  ...
  console.log(
    `ðŸ“¦ Encontradas ${venueCategories.length} categorÃ­as en Venue_Category`
  );

  // 2. Obtener categorÃ­as existentes en Tickets para preservar datos
  const existingTickets = await dynamodb
    .query({
      TableName: "Tickets",
      IndexName: "eventIdIndex",
  ...
  const existingBoletas =
    existingTickets.Items && existingTickets.Items.length > 0
      ? existingTickets.Items[0].boletas || []
      : [];

  // Crear mapa de categorÃ­as existentes por ID
  const existingCategoriesMap = {};
  for (const boleta of existingBoletas) {
    if (boleta.id) {
      existingCategoriesMap[boleta.id] = boleta;
  ...
  // 3. Transformar categorÃ­as a formato de Tickets
  const ticketCategories = [];

  for (const cat of venueCategories) {
    // Obtener conteo de seats si hasSeating
    let totalSeats = cat.totalSeats || 0;
    if (hasSeating && cat.categoryId) {
    ...
    // Verificar si existe en Tickets para preservar datos
    const existingCat = existingCategoriesMap[cat.categoryId];
    ...
    ticketCategories.push({
      categoria: cat.name,
      id: cat.categoryId,
      cantidadTickets: totalSeats,
      avaliableCapacity: avaliableCapacity,
      reservedTickets: reservedTickets,
      soldTickets: soldTickets,
      moneda: cat.currency || "COP",
      costo: existingCat ? existingCat.costo || 0 : 0,
      valor: cat.ticketPrice || 0,
      descripcion: cat.description || "",
      imgboleta: existingCat ? existingCat.imgboleta || "" : "",
      gateId: cat.gateId || "",
      distributionId: existingCat
        ? existingCat.distributionId || uuidv4()
        : uuidv4(),
      distributionCreateDate: existingCat
        ? existingCat.distributionCreateDate || now
        : now,
    });

    if (existingCat && (reservedTickets > 0 || soldTickets > 0)) {
      console.log(
        `ðŸ”„ Preservando datos para ${cat.name}: ${soldTickets} vendidos, ${reservedTickets} reservados`
      );
    }
  }

  console.log(
    `ðŸŽ« Preparadas ${ticketCategories.length} categorÃ­as para Tickets`
  );

  // 4. Si no hay categorÃ­as en Venue_Category pero hay eventId, verificar Tickets existentes
  if (ticketCategories.length === 0 && eventId) {
    console.log(
      "âš ï¸ No hay categorÃ­as en Venue_Category, verificando Tickets..."
    );

    if (existingBoletas.length > 0) {
      console.log(
        `ðŸ“‹ Manteniendo ${existingBoletas.length} categorÃ­as existentes en Tickets`
      );
      return {
        action: "preserved",
        categoriesCount: existingBoletas.length,
        message:
          "No hay categorÃ­as en Venue_Category, se mantienen las existentes en Tickets",
      };
    }
  }

  // 5. Actualizar tabla Tickets con sincronizaciÃ³n completa
  if (eventId && ticketCategories.length > 0) {
    ...
    console.log("âœ… SincronizaciÃ³n con Tickets completada");

    // ðŸŽ« SINCRONIZAR TICKETS DISTRIBUTION
    // Actualizar o crear las boletas individuales en TicketsDistribution
    let distributionSync = null;
    if (result && result.ticketId) {
      try {
        ...
        console.log(
          `âœ… TicketsDistribution sincronizado: ${distributionSync.created} creados, ${distributionSync.updated} actualizados`
        );
      } catch (distError) {
        console.error("âŒ Error sincronizando TicketsDistribution:", distError);
        // No fallar toda la operaciÃ³n si falla la sincronizaciÃ³n de distributions
      }
    }
    ...
    return {
      ...
      distributionSync,
      message:
        "CategorÃ­as sincronizadas automÃ¡ticamente con Tickets y TicketsDistribution",
    };
  }

  console.log(
    "âš ï¸ No se requiere actualizaciÃ³n de Tickets (sin eventId o sin categorÃ­as)"
  );
  return {
    action: "skipped",
    categoriesCount: 0,
    message: "No hay categorÃ­as o eventId para sincronizar",
  };
}
