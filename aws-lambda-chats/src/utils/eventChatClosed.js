const { isEventRoom } = require("./chatRoomAdmin");
...
function resolveDisplayEventStatus(eventInfo) {
  const normalized = String(eventInfo?.estatus || eventInfo?.status || "").toLowerCase().trim();
...
function isEventItemClosed(eventItem) {
  if (!eventItem) return false;
  const status = resolveDisplayEventStatus(eventItem);
  return status === "finalizado" || status === "cancelado";
}

async function loadEventForRoom(docClient, room) {
  const eventId = String(room?.event || room?.eventId || "").trim();
...
async function isEventChatRoomClosed(docClient, room) {
  if (!isEventRoom(room)) return false;
  const eventItem = await loadEventForRoom(docClient, room);
  return isEventItemClosed(eventItem);
}

async function assertEventChatRoomIsOpen(docClient, room) {
  if (!room || !isEventRoom(room)) return room;
  const closed = await isEventChatRoomClosed(docClient, room);
  if (closed) {
    const err = new Error("El chat del evento está cerrado. Solo puedes ver el historial.");
    err.statusCode = 403;
    throw err;
  }
  return room;
...
module.exports = {
  ...
  isEventItemClosed,
  isEventChatRoomClosed,
  assertEventChatRoomIsOpen,
};
