...
exports.getStaffView = exports.getEventsStaffSummary = exports.getAdminView = exports.saveAssignments = void 0;
...
const PK = "userId";
const SK = "sk";
const doc = lib_dynamodb_1.DynamoDBDocumentClient.from(new client_dynamodb_1.DynamoDBClient({ region: process.env.DYNAMODB_REGION || process.env.AWS_REGION || "us-east-2" }));
 ...
 */
async function resolveUserProfileImageUrl(fotoPerfilUrl, _platform) {
    if (!fotoPerfilUrl || typeof fotoPerfilUrl !== "string")
        return null;
    if (isHttpUrl(fotoPerfilUrl))
        return fotoPerfilUrl;
    try {
        return await (0, s3_request_presigner_1.getSignedUrl)(s3, new client_s3_2.GetObjectCommand({ Bucket: PROFILE_BUCKET, Key: fotoPerfilUrl }), { expiresIn: 3600 });
    }
    catch {
        return fotoPerfilUrl;
    }
...
function onError(error) {
    ...
    return replyErr(code, message, process.env.NODE_ENV === "development" ? { details: String(error) } : undefined);
}
// ----- DynamoDB -----
const sk = (eventId, gateId) => `${eventId}#${gateId}`;
function toStaffAccessItem(row) {
    return {
...
async function queryByEventId(eventId) {
    ...
    return (r.Items ?? []).map(toStaffAccessItem);
}
...
async function getEventsByIds(eventIds) {
    ...
    for (let i = 0; i < ids.length; i += BATCH_GET_LIMIT) {
        ...
        const rows = (r.Responses?.[EVENTS_TABLE] ?? []);
        rows.forEach((row) => {
            const id = toStr(row.id) ?? "";
            if (id)
                out.set(id, row);
...
async function getVenuesByIds(venueIds) {
    const ids = [...new Set(venueIds)].filter(Boolean);
    if (ids.length === 0 || !VENUES_TABLE)
        return new Map();
    const out = new Map();
    for (let i = 0; i < ids.length; i += BATCH_GET_LIMIT) {
        const chunk = ids.slice(i, i + BATCH_GET_LIMIT);
        ...
        const rows = (r.Responses?.[VENUES_TABLE] ?? []);
        rows.forEach((row) => {
...
async function saveToDynamo(keysToDelete, itemsToPut) {
    const run = async (ops) => {
        if (ops.length === 0)
            return;
        let unprocessed = ops;
        for (let attempt = 0; attempt < 2; attempt++) {
            const result = await doc.send(new lib_dynamodb_1.BatchWriteCommand({ RequestItems: { [TABLE]: unprocessed } }));
    ...
    for (let i = 0; i < keysToDelete.length; i += MAX_BATCH) {
        await run(keysToDelete.slice(i, i + MAX_BATCH).map((k) => ({
            DeleteRequest: { Key: { [PK]: k.userId, [SK]: k.sk } },
        })));
    }
    for (let i = 0; i < itemsToPut.length; i += MAX_BATCH) {
        await run(itemsToPut.slice(i, i + MAX_BATCH).map((item) => ({
            PutRequest: {
                Item: {
                    [PK]: item.userId,
                    [SK]: item.sk,
...
function validateBody(body) {
    ...
    if (!eventId || typeof eventId !== "string")
        throw new Error("MISSING_OR_INVALID_EVENT_ID");
    if (!venueId || typeof venueId !== "string")
        throw new Error("MISSING_OR_INVALID_VENUE_ID");
    if (!Array.isArray(accessControl) || accessControl.length === 0)
        throw new Error("ACCESS_CONTROL_REQUIRED");
    accessControl.forEach((gate, i) => {
        const g = gate;
        const ids = g?.assignedUsers;
        if (!Array.isArray(ids) || ids.length === 0)
            throw new Error(`GATE_MUST_HAVE_AT_LEAST_ONE_USER: gateIndex=${i}`);
        ids.forEach((id) => {
            if (typeof id !== "string" || !id.trim())
                throw new Error(`INVALID_USER_ID: gateIndex=${i}`);
        });
    });
    const allIds = accessControl.flatMap((g) => g.assignedUsers);
    if (allIds.length !== new Set(allIds).size)
        throw new Error("USER_CANNOT_BE_IN_TWO_GATES_SAME_EVENT");
}
// ----- Handlers -----
/** 1. Admin guarda/edita control de acceso → guarda en DB y notifica a cada usuario del payload */
async function saveAssignmentsHandler(event) {
    let body;
    try {
    ...
    try {
        ...
        if (msg.startsWith("GATE_MUST_HAVE"))
            return replyErr(400, "Cada puerta debe tener al menos un usuario asignado.", { code: msg });
        if (msg === "USER_CANNOT_BE_IN_TWO_GATES_SAME_EVENT")
            return replyErr(400, "Un usuario no puede estar en dos puertas en el mismo evento.", { code: msg });
    ...
    try {
        ...
        const toPut = body.accessControl.flatMap((gate) => gate.assignedUsers.map((userId) => ({
            userId,
 ...
 */
function eventSummaryForStaff(raw, venueRow) {
...
async function getStaffViewHandler(event) {
    const userId = event.requestContext?.authorizer?.userId ??
        event.pathParameters?.userId ??
        event.queryStringParameters?.userId;
    if (!userId)
        return replyErr(400, "MISSING_USER_ID");
    ...
    const ownerIds = [...new Set(eventIds.map((id) => toStr(eventsMap.get(id)?.userId)).filter(Boolean))];
    const ownersMap = await getUsersByIds(ownerIds);
    const assignments = await Promise.all(items.map(async (x) => {
        ...
        const fotoPerfil = owner
            ? await resolveUserProfileImageUrl(toStr(owner.fotoPerfilUrl), toStr(owner.PLATFORM ?? owner.platform))
            : null;
...
async function getEventsStaffSummaryHandler(event) {
    const userId = event.requestContext?.authorizer?.userId ??
        event.queryStringParameters?.userId ??
        event.pathParameters?.userId;
...
exports.saveAssignments = wrap(saveAssignmentsHandler);
exports.getAdminView = wrap(getAdminViewHandler);
exports.getEventsStaffSummary = wrap(getEventsStaffSummaryHandler);
exports.getStaffView = wrap(getStaffViewHandler);
